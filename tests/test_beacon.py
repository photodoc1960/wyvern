...
def test_regular_callbacks_detected(config, feed, sweep, mk):
    ...
    base = 1000.0
    events = [mk.syn("192.168.1.50", "203.0.113.5", 4444, base + i * 60,
                     mac="00:14:22:00:00:50", sport=50000 + i) for i in range(8)]
...
def test_irregular_traffic_not_beacon(config, feed, sweep, mk):
    ...
    events = [mk.syn("192.168.1.50", "203.0.113.5", 4444, base + g,
                     mac="00:14:22:00:00:50", sport=50000 + i) for i, g in enumerate(gaps)]
    feed(det, events)
...
def test_standard_port_excluded(config, feed, sweep, mk):
    det = BeaconDetector(config)
    base = 1000.0
    events = [mk.syn("192.168.1.50", "203.0.113.5", 443, base + i * 60,
                     mac="00:14:22:00:00:50", sport=50000 + i) for i in range(8)]
    feed(det, events)
