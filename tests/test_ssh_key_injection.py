...
def test_idle_device_ssh_fanout(config, feed, mk):
    ...
    events += [
        mk.syn("192.168.1.30", f"192.168.1.{h}", 22, 10.0 + h, mac="00:80:77:aa:bb:cc")
...
def test_many_hosts_fires_for_any_device(config, feed, mk):
    det = SshKeyInjectionDetector(config)
    events = [
        mk.syn("192.168.1.50", f"192.168.1.{h}", 22, 10.0 + h, mac="00:14:22:00:00:50", ttl=128)
...
def test_non_ssh_ignored(config, feed, mk):
    det = SshKeyInjectionDetector(config)
    events = [
        mk.syn("192.168.1.30", f"192.168.1.{h}", 23, 10.0 + h, mac="00:80:77:aa:bb:cc")
