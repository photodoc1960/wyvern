...
def test_learns_after_window():
    ...
    for i in range(15):
        ev = ConnEvent(ts=100.0 + i, src_ip="192.168.1.50", dst_ip="192.168.1.10",
                       dst_port=443, src_mac="00:14:22:00:00:50", flags="S", ttl=128)
        reg.observe(ev)
...
def test_tracks_inference_and_auth_fanout():
    ...
    reg.observe(ConnEvent(ts=1, src_ip="192.168.1.50", dst_ip="192.168.1.99",
                          dst_port=443, src_mac="00:14:22:00:00:50", flags="S"))
    learner.observe(HttpEvent(ts=2, src_ip="192.168.1.50", dst_ip="192.168.1.99",
                              dst_port=8000, method="POST", path="/v1/chat/completions",
                              src_mac="00:14:22:00:00:50"), reg)
    for h in range(10, 17):
        ev = ConnEvent(ts=3 + h, src_ip="192.168.1.50", dst_ip=f"192.168.1.{h}",
                       dst_port=22, src_mac="00:14:22:00:00:50", flags="S")
        reg.observe(ev)
        learner.observe(ev, reg)
    p = learner.get("00:14:22:00:00:50")
...
def test_persistence_round_trip(tmp_path):
    ...
    for i in range(12):
        ev = ConnEvent(ts=i, src_ip="192.168.1.50", dst_ip="192.168.1.10", dst_port=80,
                       src_mac="00:14:22:00:00:50", flags="S")
        reg.observe(ev)
        learner.observe(ev, reg)
