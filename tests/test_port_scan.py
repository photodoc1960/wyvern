...
def test_fires_above_threshold(config, feed, mk):
    ...
    events = [mk.syn("192.168.1.50", "192.168.1.10", p, 1000.0 + p * 0.1,
                     mac="00:14:22:00:00:50") for p in range(1, 61)]
...
def test_quiet_below_threshold(config, feed, mk):
    det = PortScanDetector(config)
    events = [mk.syn("192.168.1.50", "192.168.1.10", p, 1000.0 + p,
                     mac="00:14:22:00:00:50") for p in range(1, 40)]
