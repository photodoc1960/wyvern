...
- **Within design scope to fix?** No. Implementing the timing-based detection path requires TCP stream reassembly and inter-packet gap analysis on long-lived HTTPS connections — a significant architectural addition beyond Wyvern's current per-packet event model. TLS interception is not needed (timing analysis is metadata-only), but stream reassembly is, which is outside the current design scope.
- **Candidate future hardening:** A stream-timing pass tracking inter-packet gaps on long-lived HTTPS connections from non-GPU devices, flagging streams matching token-streaming cadences as a corroborating signal alongside the existing port/URL detector. This is technically compatible with passive capture but requires substantial redesign of the packet-event model.
